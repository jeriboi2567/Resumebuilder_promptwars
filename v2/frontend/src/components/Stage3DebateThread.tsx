...
import { MessageSquare, ArrowRight, Quote, RefreshCw, CheckCircle2, AlertCircle } from 'lucide-react';
...
export const Stage3DebateThread: React.FC<Stage3DebateThreadProps> = ({ debateState }) => {
  const getAgentColor = (agentName: string) => {
    switch (agentName) {
      case 'Technical Agent':
        return { bg: 'bg-blue-950/60', border: 'border-blue-500/40', text: 'text-blue-400', badge: 'bg-blue-500/20 text-blue-300' };
      case 'HR / Culture Agent':
        return { bg: 'bg-emerald-950/60', border: 'border-emerald-500/40', text: 'text-emerald-400', badge: 'bg-emerald-500/20 text-emerald-300' };
      case 'Hiring Manager Agent':
        return { bg: 'bg-purple-950/60', border: 'border-purple-500/40', text: 'text-purple-400', badge: 'bg-purple-500/20 text-purple-300' };
      case 'Skeptic Agent':
        return { bg: 'bg-rose-950/60', border: 'border-rose-500/40', text: 'text-rose-400', badge: 'bg-rose-500/20 text-rose-300' };
      default:
        return { bg: 'bg-slate-800', border: 'border-slate-700', text: 'text-slate-300', badge: 'bg-slate-700 text-slate-200' };
    }
  };
  ...
  return (
    <div className="space-y-6 bg-slate-800/90 border border-slate-700 rounded-xl p-6 shadow-xl">
      ...
      {/* Stance Delta Summary Table (Before vs After Debate) */}
      ...
      <div className="space-y-4 pt-2">
        ...
        {debateState.turns.map((turn, index) => {
          ...
          return (
            <div
              key={index}
              className={`p-4 rounded-xl border ${colors.bg} ${colors.border} space-y-3 relative shadow-md transition-all hover:border-slate-500`}
            >
